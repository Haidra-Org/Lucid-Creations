...
#	print_debug(body)
	...
	var error = request("https://dev.stablehorde.net/api/v2/generate/async", headers, false, HTTPClient.METHOD_POST, body)
	if error != OK:
...
func check_request_process(operation := OngoingRequestOperations.CHECK) -> void:
	...
	var url = "https://dev.stablehorde.net/api/v2/generate/check/" + async_request_id
	var method = HTTPClient.METHOD_GET
	if operation == OngoingRequestOperations.GET:
		url = "https://dev.stablehorde.net/api/v2/generate/status/" + async_request_id
